...
This can be useful if PDFs being sent through the application are always located in the same directory.](Readme_Images/Image_1.png)
...
![It is important to let the application know about entries expected to be found in tables. That way, the program will not be
 ...
 is more than one option for correction.](Readme_Images/Image_2.png)
...
it in the left hand column as is shown with “*X”.](Readme_Images/Image_3.png)
...
is not required that these rows be ignored, but it can speed up the conversion and make the generated table look cleaner.](Readme_Images/Image_4.png)
...
entry on its own line so that they are written above or below each other within the cell.](Readme_Images/Image_7)
...
![After the PDF is selected and the correct number of rows and columns are selected (Currently, every table on each page must currently have the same number of rows and columns),
 click “Generate Tables”. The progress bar at the bottom should start to fill up. Once complete, the generated table will be displayed.](Readme_Images/Image_8.png)
...
entries that are separated by commas will also be expanded. “V” can be entered into a cell to copy an entry from above into the cell containing “V”.](Readme_Images/Image_9.png)
...
are not shown by default, hold control and roll the middle mouse wheel until images are displayed.](Readme_Images/Image_10.png)